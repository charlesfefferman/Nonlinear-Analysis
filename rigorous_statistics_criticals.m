function [TC_min, TC_max, rhoC_min, rhoC_max, PC_min, PC_max, ZC_min, ZC_max ] = rigorous_statistics_criticals( T,pv,pL,MW)
%This function requires the best fit parameters as input and it performs a
%rigorous determination of the confidence region.  

% [TC_fit, pc_fit, A_fit, b_fit] = towhee_rectilinear(T,pv,pL,sv,sL);
...
RHS = sigma * (n + p * (finv(alpha,p,n-p)-1)); % No 0.95^p because I want the true 95% (though sometimes it will be 0.95^2 or 0.95^4)
...
% PC_ext_D_low = PC_from_Rackett_D(TC_ext,pc_ext,VC_ext,A_ext,b_ext,TR,D_low);
% PC_ext_D = PC_from_Rackett_D(TC_ext,pc_ext,VC_ext,A_ext,b_ext,TR,D);
% PC_ext_D_high = PC_from_Rackett_D(TC_ext,pc_ext,VC_ext,A_ext,b_ext,TR,D_high);
% 
% % PC_ext = [PC_ext_D_low, PC_ext_D, PC_ext_D_high];
% PC_ext = PC_ext_D;
% 
% PC_max = max(PC_ext);
% 
...
truly_rigorous_criticals_uncertainty
